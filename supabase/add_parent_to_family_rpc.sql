...
create or replace function list_my_family_members()
returns table (
  id uuid,
  user_id uuid,
  role text,
...
as $$
  select fm.id,
         ...
         fm.is_active
  from public.family_members as fm
  join public.profiles as p on p.id = fm.user_id
  join auth.users as au on au.id = fm.user_id
  where fm.family_id = user_family_id()
  order by fm.created_at asc
$$;

grant execute on function list_my_family_members() to authenticated;
