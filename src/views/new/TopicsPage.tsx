...
export function TopicsPage() {
  ...
  const { rawTasks, taskProgress, rawSubjects, rawTopics, kids, loading, error, refresh } = useData();
  const toast = useToast();
  const { selectedChildId } = useSettingsStore();
  const childId = selectedChildId || kids?.[0]?.id || '';

  // Synchronously compute subject and topics from global cache
  const subject = useMemo(() => subjectId ? (rawSubjects.find(s => s.id === subjectId) || null) : null, [subjectId, rawSubjects]);
  const topics = useMemo(() => subjectId ? rawTopics.filter(t => t.subject_id === subjectId) : rawTopics, [subjectId, rawTopics]);
  ...
  async function handleSubmit(data: any) {
    try {
      ...
      refresh(); // Background sync via DataContext
    } catch {
