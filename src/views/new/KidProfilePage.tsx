...
export function KidProfilePage() {
  ...
  const { kids, subjects, topics, rawTopics, loading, refresh } = useData();
  ...
  if (!kid) return (
    <div className="text-center py-20">
      ...
      <button onClick={() => router.push('/kids')} className="mt-3 text-violet-600 text-sm font-semibold">← Back to Kids</button>
    </div>
  );

  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <button onClick={() => router.push('/kids')} className="flex items-center gap-2 text-sm text-gray-400 hover:text-violet-600 transition-colors font-medium">
          ...
          {tab === 'subjects' && (
            <div className="space-y-3">
              ...
              {kidSubjects.length === 0
                ...
                : kidSubjects.map((s) => (
                  <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => router.push(`/subjects/${s.id}/topics`)}>
          ...
          {tab === 'topics' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {kidTopics.length === 0
                ...
                : kidTopics.slice(0, 6).map((t, i) => {
                    const rawTopic = rawTopics.find(rt => rt.id === t.id);
                    if (!rawTopic) return null;
                    return <TopicCard key={t.id} topic={rawTopic} index={i} completed={t.completed} />;
                  })
              }
            </div>
          )}
