...
import { ArrowLeft, GraduationCap, Loader2, LogIn, Mail } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { sendEmailOtp, verifyOtp, signInWithGoogle } from '../services/supabase';
          ...
          {step === 'email' ? (
            ...
            <>
              ...
              <p className="text-gray-500 text-sm mb-6">
                We sent a 6-digit verification code to <span className="font-medium text-gray-800">{email}</span>
              </p>

              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5" htmlFor="login-otp">
                    Verification Code
                  </label>
                  <input
                    id="login-otp"
                    ...
                    required
                    maxLength={6}
                    className="w-full px-4 py-3 text-center tracking-widest text-lg font-mono bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100 transition-all"
                  />
                </div>
