          ...
          {groupBy === 'subject' ? (
              ...
              <div key={subject.id}>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {subTopics.map((topic: any, idx: number) => (
                    <TopicButton key={topic.id} topic={topic} idx={idx} navigate={navigate} showSubject={false} />
                  ))}
                </div>
              </div>
            ))
            ...
            <div className="grid grid-cols-1 gap-2">
              {filtered.map((topic: any, idx: number) => (
                <TopicButton key={topic.id} topic={topic} idx={idx} navigate={navigate} showSubject={true} />
              ))}
            </div>
          )}
        </div>
...
function TopicButton({ topic, idx, navigate, showSubject }: { topic: any, idx: number, navigate: any, showSubject: boolean }) {
  return (
    <button
